...
Действия в игре занимают реальное время, которое конвертируется в игровое. Игрок не может совершать другие действия, пока текущее не завершится.
...
Реализуются через глобальные квесты "Сессия". Раз в 30 дней активируется цепочка квестов "Сдать Экзамен 1", "Сдать Экзамен 2". Условие выполнения: Знания > 50% + Трата энергии.
...
- **Слой 2: Верхняя Панель (Top Bar)**
  ...
  - Top Left (Ресурсы): Вертикальный стек компактных индикаторов (Иконка + Число/Полоска):
    ...
    - Деньги (Крупно)
    - Успеваемость (Книга)
  - Top Right (Квесты): Вертикальный стек круглых иконок (Quest Badges). Отображаются только Active квесты. Индикатор таймера (ободок вокруг иконки). Клик открывает модалку с деталями задания.
...
1. Верстка UI (Слои фона и HUD)
2. Quest Engine (JSON логика)
3. Интеграция квестов с ресурсами
